...
const StaffMap = ({
  ...
  return (
    <>
      ...
      <div
        key={staff.id}
        className="py-3 px-5 pb-0 h-[330px] max-w-full mx-auto rounded-[4px] overflow-hidden shadow-whiteeee z-10 bg-white border-[1px] border-[#F7F7F7]"
      >
        <div className="text-center  justify-center w-[100px] relative top-[8px] left-[167.5px] bg-[#e6e0fa] text-[#350ABC] rounded-md inline-flex gap-2 py-[6px] px-[14px]">
          <span className="text-[16px]  font-[400] tracking-[-2%] leading-[28px]">
            ${parseFloat(staff?.per_hours_rate).toFixed(0)}/hr
          </span>
        </div>
          ...
          <div className="flex flex-col mt-[30px] items-center w-full relative z-50">
            <div className="text-center flex justify-between w-full items-center font-bold text-lg mb-[21px]">
              <div className="flex justify-start items-center">
                ...
                >
                  {staff?.full_name?.length > 18
                    ? `${staff?.full_name?.slice(0, 18)}...`
                    : staff?.full_name}
                </h3>
                {staff?.id_is_verified && staff?.contact_is_verified ? (
                  <Tooltip
                    content={
                      <VerificationStatus
              ...
              <div className="flex items-center justify-center">
                ...
                <p className="ms-1 text-[14px]  font-normal text-black">
                  {parseFloat(staff?.rating).toFixed(1)}/5
                </p>
              </div>
            </div>
            <div className="text-center flex justify-between w-full items-center font-bold text-lg mb-1">
              <div className="text-center text-[14px] font-normal  text-[#989898] flex gap-2">
                <Image
                  ...
                  height={16}
                />{" "}
                <span className="flex justify-start items-center !text-[16px] !font-[400] text-[#00000080] cursor-pointer">
                  {staff?.city}
                </span>
              </div>
              <span className="text-[16px] font-[400] leading-[28px] text-[#6B6B6B]">
                {staff?.total_jobs_count} Jobs
              </span>
            </div>
          </div>
          ...
          <div className="flex justify-start items-center mt-2 w-full">
            <div className="flex items-center ">
              <p
                ...
                className="relative top-[5px] leading-[24px] py-[4px] px-[10px] !text-[11px] font-normal inline-flex items-center rounded-[100px] bg-[#F6F6F6] whitespace-nowrap overflow-hidden text-ellipsis text-sm"
              >
      ...
      <div
        className={`flex justify-around items-center flex-col mt-0 ${
          staff?.alreadyInvited && "h-[110px]"
        } mt-[32px] relative bottom-[142.9px] w-full space-x-2  pt-2 pb-[17.5px] rounded-bl-[4px] rounded-br-[4px] border-x-[1px] border-[#F7F7F7] ${
          isJobDetailInvite && "h-[21.6%]"
        }`}
      >
        {openBreakDown && (
          <img
            src="/images/shadow pic.png"
            alt=""
            className="absolute bottom-[262px] z-20"
          />
        )}
        {openBreakDown && (
          <div className="!h-[10.8rem] !m-0 absolute z-[99]  bg-[#fff] text-[#2C2240] !pb-4 w-full  bottom-[84px] space-y-2 capitalize">
            <div
              onClick={handleCloseCastBreakdown}
              className="w-full flex justify-center items-center cursor-pointer"
            >
              <span className="rotate-180 relative top-[8px]">
                <svg
                  width="24"
                  height="25"
                  viewBox="0 0 24 25"
            ...
            <div className="text-[12px] font-[400] leading-[24px] tracking-[-2%] px-3 flex justify-between items-center py-2 border-b border-[#F3F0FF]">
              <h2>Worker fee per hour</h2>
              <h2 className="!font-[500] !text-[12px] !leading-[24px] !tracking-[-2%]">
                ${parseFloat(staff?.per_hours_rate)}
              </h2>
            </div>
            <div className="text-[12px] font-[400] leading-[24px] tracking-[-2%] px-3 flex justify-between items-center py-2 border-b border-[#F3F0FF]">
              <h2>Total Numbers of hours</h2>
              <h2 className="!font-[500] !text-[12px] !leading-[24px] !tracking-[-2%] lowercase">
                {calculateTotalHours(
                  jobApiData
                    ? jobApiData?.working_times?.timing
                    : selectedService?.working_hours?.timing
                )}
                h
              </h2>
            </div>
            <div className="text-[12px] font-[400] leading-[24px] tracking-[-2%] px-3 flex justify-between items-center py-2">
              <h2>Worker fee calculation</h2>
              <h2 className="!font-[500] !text-[12px] !leading-[24px] !tracking-[-2%] lowercase">
                <small className="mr-2 !text-[10px] text-[#9F9F9F] font-[400] leading-[24px] tracking-[-0.2px]">
                  {calculateTotalHours(
                    jobApiData
                      ? jobApiData?.working_times?.timing
                      : selectedService.working_hours.timing
                  )}{" "}
                  h ✕ ${parseFloat(staff?.per_hours_rate)}
                </small>{" "}
                $
                {calculateTotal(
                  staff?.per_hours_rate,
                  calculateTotalHours(
                    jobApiData
                      ? jobApiData?.working_times?.timing
                      : selectedService.working_hours.timing
        ...
        <div className="mt-[31px] w-[86%]">
          <div
            className={`space-x-[20px] !m-0 flex justify-between  ${
              sendInvite ||
              isInvited ||
              (staff?.alreadyInvited && "!justify-center")
            } items-center `}
          >
            <div
              style={{
                right: isInvited ? "right-[0px]" : "",
                position: "relative",
              }}
              className={`relative text-[#350ABC] font-[500] text-[26px] z-[999] ${
                staff?.alreadyInvited || isInvited ? "!right-[0px] top-1" : ""
              }`}
            >
              $
              {calculateTotal(
              ...
              <span className="!text-[12px] !leading-[14.63px] !mr-[4px] !text-[#6B6B6B]">
                Total.
              </span>
            </div>

            {!staff?.alreadyInvited && !isJobDetailInvite && (
              <button
                onClick={handleSendInvite}
                type="button"
                className={`!text-[#F3F0FF]   ${
                  sendInvite || isInvited || staff?.alreadyInvited
                    ? "bg-[#774DFD]"
                    : "bg-[#2c2240]"
                } rounded-[4px] text-[13px] font-[400] w-[120px] py-[10px] text-center inline-flex items-center tracking-[-2%] leading-[24px] me-2 
              ...
              >
                ...
                >
                  {sendInvite || isInvited || staff?.alreadyInvited ? (
                    ...
                    <>
                      {"Send Invite "}
                      <small className="relative top-[1px] text-md ml-2">
                        ➤
                      </small>
                    </>
                  )}
                  ...
                  <span className="ml-[9.33px]">
                    {!sendInvite ||
                      (isInvited && (
                        ...
                        >
                          <path
                            d="M15.1666 1.33337L7.83325 8.66671M15.1666 1.33337L10.4999 14.6667L7.83325 8.66671M15.1666 1.33337L1.83325 6.00004L7.83325 8.66671"
                            stroke="#F3F0FF"
                            stroke-width="0.7"
                            strokeLinecap="round"
                            strokeLinejoin="round"
