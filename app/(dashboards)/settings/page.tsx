...
const Page = () => {
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    const updatedData: UpdatedData = prepareUpdatedData(profile, originalProfile, showPasswordFields, password, confirmPassword);
        ...
        isLoading ?
          ...
          <div className="my-12 w-[820px]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-[28px] mb-4">
              <div>
                <label className="text-[#2C2240] text-[16px] font-[500] leading-normal">Full Name</label>
              ...
              <div>
                <label className="text-[#2C2240] text-[16px] font-[500] leading-normal">Phone Number</label>
                <div className="relative mt-[8px] w-full">
                  {/* Icon positioned to the left */}
                  <BsTelephone className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400" size={20} />
            ...
            {/* Company Name */}
            <div className="mb-4">
              <label className="text-[#2C2240] text-[16px] font-[500] leading-normal">Company</label>
              <div className="relative mt-[8px] w-full">
                {/* Icon positioned to the left */}
                <PiBuilding className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400" size={20} />
            ...
            <div className="mb-4">
              ...
              <div className='flex !gap-[28px]'>
                <div className="relative w-full">
                  <p className="text-[#2C2240] text-[16px] font-[500] leading-normal">Email</p>
                  <p className="text-sm text-[#6B6B6B] font-[500] leading-[19px] text-[16px] mt-2">Used to login to your account <span className='!text-[#161616]'>(Read Only)</span></p>
                </div>
                <div className="relative w-full">
                  {/* Icon positioned to the left */}
                  <VscMail className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400" size={20} />
            ...
            {/* Password Section */}
            <div className={`${showPasswordFields && "bg-[#FFF] !mb-6 px-[20px]"} mb-2 py-[40px] relative`}>
              <div className='flex  justify-between items-center w-full'>
                {!showPasswordFields && <div
                ...
                {!showPasswordFields && <div className='cursor-pointer text-[#161616] text-[14px] font-[400] leading-[26px] bg-[#FFF] border-[1px] border-[#6B6B6B] rounded-[4px] py-[12px] px-[32px] flex justify-center items-center gap-2' onClick={() => setShowPasswordFields(!showPasswordFields)}>Update Password</div>}
              </div>
              ...
              {showPasswordFields && (
                <div className="flex flex-row gap-[28px]">
                  <div className='flex flex-row gap-2'>
                    <div>
                      <label className="text-[#2C2240] text-[16px] font-[500] leading-normal">Old Password</label>
                          ...
                          <div className="relative mt-[7px]">
                            <input
                             style={{
                              ...
                              className={`defaultsearch shadow-lg ${!!passwordVerificationError && "!border-[1px] !border-red-200"} w-[240px] pb-[20px] pt-[20px] pl-[12px] min-h-[52px] focus:outline-blue-200`}
                            ...
                            >
                              {showPassword ? <BiHide color='#9F9F9F' size={18} /> : <BiShow color='#9F9F9F' size={18} />}
                            </div>


                          </div>
                          <div className='!text-red-400 text-[12px] relative right-[29px] bottom-2'>{!!passwordVerificationError && 'Incorrect password! Try again'}</div>
                        </div>
                        {isVerifyPasswordLoading
                          ?
                          <><div className='loader_login relative top-[4px] left-[20px]'></div></> : <>
                            {
                              ...
                              <div className='flex justify-start items-center'>
                                <button
                                  ...
                                  className={`ml-[32px] !bg-transparent bottom-[2px] hover:!bg-transparent flex justify-center cursor-pointer !text-[16px] items-center w-[100%] ${passwordVerificationError ? '!cursor-default !bottom-[12px]' : 'text-[#350ABC] cursor-pointer'}  !text-[#350ABC] !text-[16px] !font-[400] !leading-[31px] disabled:!text-gray-500 disabled:!cursor-not-allowed p-0 bg-transparent border-none`}
                                  onClick={() => {
                                    verifyPassword(profile.currentPassword);
                                  }}
                                >
                                  <span className='cursor-pointer'>{'Verify old password'}</span>
                                </button>
                                <p
                  ...
                  {showNewPasswordFields && (
                    <div>
                      <label className="text-[#2C2240] text-[16px] font-[500] leading-normal">New Password</label>
                      <div className="relative w-[240px]">
                        ...
                        >
                          {showNewPassword ? <BiHide color='#9F9F9F' size={18} /> : <BiShow color='#9F9F9F' size={18} />}
                        </div>
                      </div>
                      ...
                      {passwordError === "Password must include lowercase, uppercase, number, and special character." && (
                        <ul className="text-[12px] font-[400] w-[120%] leading-[31px] mt-[10px]">
                          <li className={`password-check flex justify-start items-center ${/[a-z]/.test(password) ? "text-green-500" : "text-[#C20000]"}`}>
                            {/[a-z]/.test(password) ? (
                              <span className="mr-1 text-green-500"><MdOutlineDone /></span>
                            ) : (
                              <span className="mr-1 text-[#C20000]"><RxCross2 /></span>
                            )}
                          ...
                          <li className={`password-check flex justify-start items-center ${/[0-9]/.test(password) ? "text-green-500" : "text-[#C20000]"}`}>
                            {/[0-9]/.test(password) ? (
                              <span className="mr-1 text-green-500"><MdOutlineDone /></span>
                            ) : (
                              <span className="mr-1 text-[#C20000]"><RxCross2 /></span>
                          ...
                          <li className={`password-check flex justify-start items-center ${/[!@#$%^&*(),.?":{}|<>]/.test(password) ? "text-green-500" : "text-[#C20000]"}`}>
                            {/[!@#$%^&*(),.?":{}|<>]/.test(password) ? (
                              <span className="mr-1 text-green-500"><MdOutlineDone /></span>
                            ) : (
                              <span className="mr-1 text-[#C20000]"><RxCross2 /></span>
                  ...
                  {showNewPasswordFields && (
                    <div>
                      <label className="text-[#2C2240] text-[16px] font-[500] leading-normal">Confirm New Password</label>
                      <div className="relative w-[239px]">
                        ...
                        >
                          {showConfirmPassword ? <BiHide color='#9F9F9F' size={18} /> : <BiShow color='#9F9F9F' size={18} />}
                        </div>
                      </div>
                      {confirmPasswordError && <span className='text-[#C20000] text-[12px] font-[400]'>{confirmPasswordError}</span>}
                    </div>
                  )}
                </div>
              )}
              ...
              {isValid ?
                <>
                  <button
                    ...
                    className={`text-[18px] cursor-pointer font-[400] py-[12px] flex items-center text-[#F3F0FF] px-[32px] rounded-[4px] transition-transform duration-150 ease-in-out transform active:scale-95  ${isFormModified || isValid ? 'bg-[#774DFD]' : 'bg-[#ddd7fc] cursor-not-allowed'
                      }`}
                  >
                    Update
                  </button>
