...
const formatWorkingTimes = (workingTimes: any) => {
  return Object.entries(workingTimes).map(([dateKey, timeSlots]: any) => (
    <div
      key={dateKey}
...
//     useState(false);
  ...
  useEffect(() => {
    const GetOffers = async () => {
      try {
        const response = await axiosCall.post(`/invitation/fetchOffers`, {
        ...
        if (response?.data?.data?.offers) {
          ...
          if (
            ...
            response?.data?.data?.offers?.forEach((offer: any) => {
  ...
  useEffect(() => {
    const getJobDetailsApi = async () => {
      ...
      try {
        const newData = await axiosCall.post(`/client/payment-request`, body);
        setReleaseData(newData?.data?.data);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchDataIfNeeded = async () => {
      ...
      try {
        ...
        // Fetch data from API
        const newData = await axiosCall.post(apiUrl, body);
        setLoading(false);
        ...
        if (newData?.data?.data) {
          ...
          setJobData(newData?.data?.data); // Update state with fetched data
          setData(newData?.data?.data); // Update state with fetched data
        }
      } catch (error) {
  ...
  }, [jobId, jobData?.id]); // Add dependencies to ensure effect is rerun if necessary
    ...
    <div className="relative flex flex-row gap-5 bg-[#F6F9FC] h-[100%]">
      ...
      <div className="grow h-full">
        {loading ? (
          ...
          <div
            className={`!bg-[#FFF] flex-col rounded-tl-[29px] rounded-[12px] flex gap-[16px] !p-8`}
          >
        ...
        {jobDetailData?.status === "open" && (
          <>
            ...
            <div className="bg-[#FFF] rounded-[12px]">
              ...
              {/* h-[calc(100vh-430px)] */}
              <div className="h-[100%] overflow-y-auto max-w-full mx-auto bg-[#FFF] rounded-bl-[24px] rounded-br-[24px]">
                {/* <JobDetailsTabs
                  jobId={jobId}
                  setOffers={setOffers}
                  offers={offers}
          ...
          >
            ...
            <button
              ...
              onClick={() => {
                if (releaseData?.release_status === "release_requested") {
                  navigate(`/events/detail/${jobId}/payment-request`);
                }
              }}
            ...
            {releaseData?.release_status === "release_pending" && (
              <div className="rounded-[8px] mt-4 p-2 bg-[#F9F7FF] w-[95%] min-h-[52px] flex flex-row items-center justify-start gap-4">
                <img
                  src="/images/event/exclamation.svg"
