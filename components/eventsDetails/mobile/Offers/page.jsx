    ...
    <>
      ...
      >
        <DialogContent
          ...
          className="w-full max-w-full max-h-[100dvh] bg-transparent border-none h-[100dvh] z-[299] overflow-y-auto px-2"
        >
          <DialogHeader hidden>
          ...
          <div>
            <GalleryContent
              images={
                offerSelected?.worker?.gallery?.length > 0
              ...
              isSelected={
                selectedTalentsLocal.some(
                  (selected) => selected.id === offerSelected?.worker?.id
                ) || offerSelected?.worker?.alreadyInvited
              }
              onToggleSelect={() =>
                !offerSelected?.worker?.alreadyInvited &&
                setSelectedTalentsLocal((prev) =>
                  prev.some((item) => item.id === offerSelected?.worker?.id)
                    ? prev.filter(
                        (item) => item.id !== offerSelected?.worker?.id
                      )
                    : [
                        ...prev,
                        {
                          id: offerSelected?.worker?.id,
                          profile_pic: offerSelected?.worker?.profile_pic,
                        },
                      ]
                )
              }
              inviteId={offerSelected?.invite_id}
